...
const MessageToast = memo(({
    ...
    return (
        <div className={`${TOAST_POSITION} px-5 py-3 rounded-2xl shadow-2xl text-white text-sm font-medium ${message.type === 'success'
...
const PermanentSuccessMessage = memo(({
    competitionName,
    onClose
}: {
...
const CompetitionHeader = memo(({
    ...
    <div className="flex items-center justify-between w-full flex-wrap gap-3 border-b border-gray-100 dark:border-gray-700/60 pb-2">
        ...
        <button
            ...
            className={`flex items-center gap-2 px-4 py-2 text-white text-xs font-bold rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 ${isValidated
...
const CompetitionDetails = memo(({
    competition,
    onValidate,
    priority = false
}: {
    competition: CompetitionInfo;
    onValidate: (c: CompetitionInfo) => Promise<boolean>;
    priority?: boolean
}) => {
    const {
        isLoading, isValidated, validationMessage, formattedStartDate,
        formattedFinishedDate, handleValidate, handleCloseMessage, clearValidationStatus
    ...
    return (
        <div className={`bg-white dark:bg-gray-800/50 rounded-2xl shadow-md overflow-hidden border border-gray-100 dark:border-gray-800 ${priority ? 'ring-2 ring-purple-500/20 shadow-lg border-purple-100' : ''
            }`}>
...
const FeuilleDeMatch = memo(() => {
    const {
        handleValidateCompetition, handleLoadMoreClick,
        competitionList, hasMore, remainingCount, isLoadingMore, hasValidatedGame,
    } = useEndGameGenerator();
