...
const RADIO_BUTTON_STYLES = {
    base: "px-4 py-2 rounded-lg border-2 transition cursor-pointer block text-center font-medium",
    selected: "bg-gradient-to-r from-blue-500 to-indigo-600 text-white border-blue-600 shadow-md",
    unselected: "bg-gray-100 text-gray-700 border-gray-300 hover:bg-gray-200 hover:border-gray-400"
} as const;
...
export const RadioButtonGroup = memo(({ options, selectedValue, onChange, name }: RadioButtonGroupProps) => (
        ...
        {options.map(({ value, label }) => (
            <label key={value} className="relative cursor-pointer">
                <input
                    ...
                    className="hidden"
                />
                <span className={`${RADIO_BUTTON_STYLES.base} ${selectedValue === value ? RADIO_BUTTON_STYLES.selected : RADIO_BUTTON_STYLES.unselected
                    }`}>
                    {label}
                </span>
            </label>
...
export const FormField = memo(({ label, children, id }: FormFieldProps) => (
