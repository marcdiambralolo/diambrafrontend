'use client';
import { useCompetitionValidation } from "@/hooks/learning/endgame/useCompetitionValidation";
import { CompetitionInfo } from "@/lib/interfaces";
import { NO_DATA_PLACEHOLDER } from "@/lib/learning/constantes";
import { formatDuration } from "@/lib/learning/functions";
import { Calendar, CheckCircle, ChevronDown, Loader2, Send, Trophy } from "lucide-react";
import { memo, useEffect, useMemo } from 'react';

// Constantes
const TOAST_POSITION = "fixed top-4 left-1/2 -translate-x-1/2 z-50";
const TOAST_DURATION = 3000;

// ============================================
// Composants atomiques
// ============================================

interface InfoRowProps {
    label: string;
    value: string | number | undefined;
    highlight?: boolean;
    icon?: React.ReactNode;
}

export const InfoRow = memo(({ label, value, highlight = false, icon }: InfoRowProps) => (
    <div className="flex justify-between items-center border-b border-gray-100 dark:border-gray-700 last:border-0 py-1.5 px-2 rounded-lg">
        <span className="font-semibold text-xs sm:text-sm text-gray-700 dark:text-gray-300 flex items-center gap-2">
            {icon && <span className="text-purple-500 flex-shrink-0">{icon}</span>}
            {label}:
        </span>
        <span
            className={`
                text-xs sm:text-sm font-mono
                ${highlight
                    ? 'text-purple-600 dark:text-purple-400 font-bold'
                    : 'text-gray-900 dark:text-gray-100'
                }
            `}
        >
            {value ?? NO_DATA_PLACEHOLDER}
        </span>
    </div>
));

InfoRow.displayName = 'InfoRow';

// ============================================
// Composants de messages
// ============================================

interface MessageToastProps {
    message: { text: string; type: string } | null;
    onClose: () => void;
}

export const MessageToast = memo(({ message, onClose }: MessageToastProps) => {
    useEffect(() => {
        if (!message) return;
        const timer = setTimeout(onClose, TOAST_DURATION);
        return () => clearTimeout(timer);
    }, [message, onClose]);

    if (!message) return null;

    const isSuccess = message.type === 'success';
    const bgGradient = isSuccess
        ? 'bg-gradient-to-r from-green-500 to-emerald-600'
        : 'bg-gradient-to-r from-red-500 to-rose-600';

    return (
        <div className={`${TOAST_POSITION} px-5 py-3 rounded-2xl shadow-2xl text-white text-sm font-medium ${bgGradient}`}>
            <div className="flex items-center gap-2">
                <span>{isSuccess ? '✅' : '⚠️'}</span>
                {message.text}
            </div>
        </div>
    );
});

MessageToast.displayName = 'MessageToast';

interface PermanentSuccessMessageProps {
    competitionName: string;
    onClose: () => void;
}

export const PermanentSuccessMessage = memo(({ competitionName, onClose }: PermanentSuccessMessageProps) => (
    <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl p-4 mb-4 shadow-lg border border-green-400 w-full">
        <div className="flex items-start gap-3">
            <CheckCircle className="w-5 h-5 text-white flex-shrink-0" />
            <div className="flex-1">
                <p className="font-bold text-sm">✅ Jeu validé avec succès !</p>
                <p className="text-xs text-green-100 mt-1">
                    Votre participation à <span className="font-semibold">{competitionName}</span> a été enregistrée.
                </p>
                <p className="text-xs text-green-100 mt-2">
                    🏆 Merci d&apos;avoir participé ! Veuillez patienter jusqu&apos;à la proclamation officielle des résultats.
                </p>
            </div>
            <button
                type="button"
                onClick={onClose}
                className="text-white/80 hover:text-white p-1 text-xs transition-colors"
                aria-label="Fermer"
            >
                ✕
            </button>
        </div>
    </div>
));

PermanentSuccessMessage.displayName = 'PermanentSuccessMessage';

// ============================================
// Composants de compétition
// ============================================

interface CompetitionHeaderProps {
    name: string;
    onValidate: () => void;
    isLoading: boolean;
    isValidated: boolean;
}

export const CompetitionHeader = memo(({ name, onValidate, isLoading, isValidated }: CompetitionHeaderProps) => {
    const buttonConfig = useMemo(() => {
        if (isLoading) {
            return { text: 'Validation...', icon: <Loader2 className="w-3.5 h-3.5 animate-spin" />, disabled: true };
        }
        if (isValidated) {
            return { text: 'Jeu validé', icon: <CheckCircle className="w-3.5 h-3.5" />, disabled: true };
        }
        return { text: 'Valider ce jeu', icon: <Send className="w-3.5 h-3.5" />, disabled: false };
    }, [isLoading, isValidated]);

    const buttonClass = useMemo(() => {
        const base = "flex items-center gap-2 px-4 py-2 text-white text-xs font-bold rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50";
        if (isValidated) {
            return `${base} bg-gray-400 dark:bg-gray-700 cursor-not-allowed`;
        }
        return `${base} bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 focus:ring-green-400`;
    }, [isValidated]);

    return (
        <div className="flex items-center justify-between w-full flex-wrap gap-3 border-b border-gray-100 dark:border-gray-700/60 pb-2">
            <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 bg-gradient-to-br from-purple-500 to-purple-700 rounded-xl flex items-center justify-center shadow-md flex-shrink-0">
                    <Trophy className="w-4 h-4 text-white" />
                </div>
                <h3 className="text-sm sm:text-base font-bold bg-gradient-to-r from-purple-700 to-purple-500 bg-clip-text text-transparent truncate max-w-[180px]">
                    {name}
                </h3>
            </div>
            <button
                type="button"
                onClick={onValidate}
                disabled={buttonConfig.disabled}
                className={buttonClass}
            >
                {buttonConfig.icon}
                {buttonConfig.text}
            </button>
        </div>
    );
});

CompetitionHeader.displayName = 'CompetitionHeader';

// ============================================

interface CompetitionStatsProps {
    startDate: string;
    finishedDate: string;
}

export const CompetitionStats = memo(({ startDate, finishedDate }: CompetitionStatsProps) => {
    const elapsedTime = useMemo(() => {
        if (!startDate || !finishedDate) return null;

        const start = new Date(startDate);
        const end = new Date(finishedDate);

        if (isNaN(start.getTime()) || isNaN(end.getTime())) return null;

        const diff = Math.floor((end.getTime() - start.getTime()) / 1000);
        return diff < 0 ? "Négatif" : formatDuration(diff);
    }, [startDate, finishedDate]);

    return (
        <div className="w-full bg-gradient-to-br from-gray-50 to-white dark:from-gray-800/30 dark:to-gray-900/30 rounded-xl p-3 border border-gray-100 dark:border-gray-700/50 mt-1">
            <InfoRow
                label="Date de début"
                value={startDate}
                icon={<Calendar className="w-3.5 h-3.5" />}
            />
            <InfoRow
                label="Date de fin"
                value={finishedDate}
                icon={<Calendar className="w-3.5 h-3.5" />}
            />
            {elapsedTime && (
                <InfoRow
                    label="Temps écoulé"
                    value={elapsedTime}
                    highlight
                    icon={<span aria-hidden="true">⏱️</span>}
                />
            )}
        </div>
    );
});

CompetitionStats.displayName = 'CompetitionStats';

// ============================================

interface CompetitionDetailsProps {
    competition: CompetitionInfo;
    priority?: boolean;
}

const CompetitionDetails = memo(({ competition, priority = false }: CompetitionDetailsProps) => {
    const {
        handleCloseMessage,
        handleClosePermanentMessage,
        handleValidate,
        isLoading,
        isValidated,
        validationMessage,
        formattedStartDate,
        showPermanentMessage,
        formattedFinishedDate,
    } = useCompetitionValidation(competition);

    const containerClass = useMemo(() => {
        const base = "bg-white dark:bg-gray-800/50 rounded-2xl shadow-md overflow-hidden border border-gray-100 dark:border-gray-800";
        if (priority) {
            return `${base} ring-2 ring-purple-500/20 shadow-lg border-purple-100`;
        }
        return base;
    }, [priority]);

    return (
        <div className={containerClass}>
            {!isValidated && (
                <MessageToast message={validationMessage} onClose={handleCloseMessage} />
            )}
            <div className="p-2 space-y-2">
                <CompetitionHeader
                    name={competition.name!}
                    onValidate={handleValidate}
                    isLoading={isLoading}
                    isValidated={isValidated!}
                />
                {isValidated && showPermanentMessage && (
                    <PermanentSuccessMessage
                        competitionName={competition.name!}
                        onClose={handleClosePermanentMessage}
                    />
                )}
                <CompetitionStats
                    startDate={formattedStartDate}
                    finishedDate={formattedFinishedDate!}
                />
            </div>
        </div>
    );
});

CompetitionDetails.displayName = 'CompetitionDetails';

// ============================================

interface LoadMoreButtonProps {
    onClick: () => void;
    remainingCount: number;
    isLoading: boolean;
}

export const LoadMoreButton = memo(({ onClick, remainingCount, isLoading }: LoadMoreButtonProps) => (
    <button
        type="button"
        onClick={onClick}
        disabled={isLoading}
        className="w-full py-3 mt-2 text-purple-600 hover:text-purple-700 hover:bg-purple-50 rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50 font-medium flex items-center justify-center gap-2"
    >
        {isLoading ? (
            <Loader2 className="w-4 h-4 animate-spin" />
        ) : (
            <ChevronDown className="w-4 h-4" />
        )}
        {isLoading ? 'Chargement...' : `Voir plus (${remainingCount} restantes)`}
    </button>
));

LoadMoreButton.displayName = 'LoadMoreButton';