...
import {
  AlertTriangle, ArrowRight, CheckCircle2, ChevronRight, Circle, Coins, Gift, ShoppingBag
} from "lucide-react";
...
const useTokenMessage = (isSufficient: boolean, availableQuantity: number, missingTokens: number) => {
  return useMemo(() => {
    if (isSufficient) {
  ...
  return (
    <div className="w-full mx-auto max-w-md dark:from-gray-900 dark:to-gray-800">
      <div className="flex flex-col items-center justify-center px-4 py-6 space-y-4">
        <Suspense fallback={<div className="h-16 w-full animate-pulse bg-gray-200 rounded-xl" />}>
          <HeaderSection />
        </Suspense>
